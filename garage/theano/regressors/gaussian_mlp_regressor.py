...
class GaussianMLPRegressor(LasagnePowered):
    ...
    def fit(self, xs, ys):
        ...
        tabular.record_tabular(prefix + 'LossBefore',
                               loss_before / batch_count)
        tabular.record_tabular(prefix + 'LossAfter', loss_after / batch_count)
        tabular.record_tabular(prefix + 'dLoss',
                               loss_before - loss_after / batch_count)
        if self._use_trust_region:
            tabular.record_tabular(prefix + 'MeanKL', mean_kl / batch_count)
